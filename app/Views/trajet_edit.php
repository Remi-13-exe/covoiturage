...
<div class="container mt-4">
    ...
    <form method="post" action="/covoiturage/trajet/edit/<?= $trajet['id'] ?>" class="w-50">
        <div class="mb-3">
            ...
            <input type="number" name="user_id" id="user_id" class="form-control" value="<?= htmlspecialchars($trajet['user_id']) ?>" required>
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="datetime-local" name="date_depart" id="date_depart" class="form-control" value="<?= date('Y-m-d\TH:i', strtotime($trajet['date_depart'])) ?>" required>
        </div>

        <div class="mb-3">
            <label for="date_arrivee" class="form-label">Date arrivée</label>
            <input type="datetime-local" name="date_arrivee" id="date_arrivee" class="form-control" value="<?= date('Y-m-d\TH:i', strtotime($trajet['date_arrivee'])) ?>" required>
        </div>

        <div class="mb-3">
            <label for="places_total" class="form-label">Nombre de places</label>
            <input type="number" name="places_total" id="places_total" class="form-control" value="<?= htmlspecialchars($trajet['places_total']) ?>" required>
        </div>
